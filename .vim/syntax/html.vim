...
" modified: Stefan Savolainen <ss@braincraft.de> added syntax highlighting for template toolkit
...
" tags
syn region  htmlString   contained start=+"+ end=+"+ contains=htmlSpecialChar,javaScriptExpression,@htmlPreproc,ttTag
syn region  htmlString   contained start=+'+ end=+'+ contains=htmlSpecialChar,javaScriptExpression,@htmlPreproc,ttTag
syn match   htmlValue    contained "=[\t ]*[^'" \t>][^ \t>]*"hs=s+1   contains=javaScriptExpression,@htmlPreproc,ttTag
syn region  htmlEndTag             start=+</+      end=+>+ contains=htmlTagN,htmlTagError,ttTag
syn region  htmlTag                start=+<[^/]+   end=+>+ contains=htmlTagN,htmlString,htmlArg,htmlValue,htmlTagError,htmlEvent,htmlCssDefinition,@htmlPreproc,@htmlArgCluster,ttTag
syn match   htmlTagN     contained +<\s*[-a-zA-Z0-9]\++hs=s+1 contains=htmlTagName,htmlSpecialTagName,@htmlTagNameCluster,ttTag
syn match   htmlTagN     contained +</\s*[-a-zA-Z0-9]\++hs=s+2 contains=htmlTagName,htmlSpecialTagName,@htmlTagNameCluster,ttTag
syn match   htmlTagError contained "[^>]<"ms=s+1

" Template Toolkit
syn keyword ttKeywords contained IF ELSE END PROCESS AND OR FILTER FOREACH IN UNLESS GET CALL SET DEFAULT INSERT INCLUDE WRAPPER
syn keyword ttKeywords contained BLOCK ELSIF SWITCH CASE WHILE FILTER USE MACRO PERL RAWPERL TRY THROW CATCH FINAL NEXT LAST RETURN STOP CLEAR META TAGS DEBUG
syn match ttOperators contained "\(=\|!\|\.\|==\)" 
syn match ttIdentifier contained "\w"
syn region ttTag matchgroup=ttTagRegion start="\[%" end="%\]" transparent contains=ttKeywords,ttOperators,ttIdentifier,htmlString
